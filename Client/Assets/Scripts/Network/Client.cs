...
public class Client : MonoBehaviour
{
    ...
    {
        {(int)SERVER_CMD.ServerPong,typeof(ServerPongRsp)},
        {(int)SERVER_CMD.ServerLoginRsp,typeof(AccountLoginRsp)},
        {(int)SERVER_CMD.ServerCreateRsp,typeof(AccountCreateRsp)},
        {(int)SERVER_CMD.ServerGetRolelistRsp,typeof(GetRoleListRsp)},
        {(int)SERVER_CMD.ServerCreateRoleRsp,typeof(RoleCreateRsp)},
        {(int)SERVER_CMD.ServerGetRoledataRsp,typeof(GetRoleDataRsp)},
        {(int)SERVER_CMD.ServerUpdateShoplistRsp,typeof(UpdateShopListRsp)},
        {(int)SERVER_CMD.ServerUpdateBaglistRsp,typeof(UpdateBagListRsp)},
        {(int)SERVER_CMD.ServerUpdateMoneyRsp,typeof(UpdateMoneyRsp)},
        {(int)SERVER_CMD.ServerRoleBuyItemRsp,typeof(RoleBuyItemRsp)},
        {(int)SERVER_CMD.ServerEnterSceneRsp,typeof(EnterSceneRsp)},
        {(int)SERVER_CMD.ServerRoleMoveSyncRsp,typeof(RoleMoveSyncRsp)},
        {(int)SERVER_CMD.ServerRoleMoveSyncAnnounce,typeof(SceneSyncAnnounce)},
        {(int)SERVER_CMD.ServerRoleExitSceneAnnounce,typeof(ExitSceneAnnounce)},
    };
    ...
    private void ConnectToServer()
    {
        try
        {
            ...
            MyEventSystem.Instance.SendEvent("OnConnect");

        }
        catch (Exception ex)
        {
            Debug.Log("IP或者端口号错误......" + ex.Message);
            staInfo = "IP或者端口号错误......";
        }
    }
    ...
    void Received()
    {
        while (true)
        {
            ...
            {
                //实际接收到的有效字节数
                int len = _socketSend.Receive(buffer, pos, buffer.Length - pos, SocketFlags.None);
                if (len == 0)
                {
                    break;
                }
