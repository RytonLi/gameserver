...
int gs::check_pack(const char* data, int len) {
    int msg_len = 0;
    if (len < PROTO_PREFIX_LEN + PROTO_SIZE_LEN) {  // 基本长度不够，表示包不完整，继续等待
        return 0;
    }

    if (memcmp(data, PROTO_PREFIX, PROTO_PREFIX_LEN) != 0) {    // 快速检验不通过，返回异常
        return -1;
    }
...
bool gs::decode(Packet* pack, const char* data, int len) {
    bool result = false;
...
// 对消息进行编码，返回编码后的长度
int gs::encode(char* buff, uint16_t cmd, const char* data, int len) {
